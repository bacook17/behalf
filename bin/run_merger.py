...
"""
Runs a N-body simulation with 2x Plummer sphere initial conditions,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--N-parts', help='REQUIRED - Total number of particles',
    ...
    parser.add_argument('--separation', help='Separation radius (in kpc)',
    ...
    sep = args.separation  # separation radius (in kpc)
    ...
    if rank == 0:
        ...
        pos_1 -= np.mean(pos_1, axis=0)
        pos_2 -= np.mean(pos_2, axis=0)
        # Set them separated by sep in x
        pos_1[:,0] += sep/2.
        pos_2[:,0] -= sep/2.
